...
Layer 1, source intelligence: `prog source add-*` creates simple profiles without hand-written seeds, while `prog discover` still turns explicit seeds or catalogs into a `SourceProfile`; `prog hints` summarizes operations, inputs, effects, and suggested next calls.
...
- [Source setup](docs/source-setup.md), including `prog source add-http` and `prog source add-cli`.
