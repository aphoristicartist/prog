...
| I9 | Stale, foreign, or incompatible cursors fail actionably and never return wrong data; valid cursors never drift to a newer payload under the same cache key. | `crates/prog-core/tests/store.rs::cursors_fail_closed_for_missing_and_expired_records`; CLI coverage in `crates/prog-cli/tests/cli.rs::{missing_call_and_expand_inputs_return_structured_errors,old_cursor_remains_bound_to_its_observation_after_cache_refresh}` |
